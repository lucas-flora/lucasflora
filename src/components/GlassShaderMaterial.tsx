...
import { chromaticAberrationChunk, reflectionChunk } from '../shaders/glslEffects';
import { bubbleMapChunk, fresnelChunk, blendModesChunk } from '../shaders/glslUtils';
...
${fresnelChunk}
${blendModesChunk}
${chromaticAberrationChunk}
${reflectionChunk}

uniform float glassOpacity;
uniform vec3 glassTint;
uniform float chromaStrength;
uniform vec2 redShift, greenShift, blueShift;
uniform float chromaBlack, chromaWhite;
uniform float reflectionStrength, reflectionClamp, fresnelPower;

void main() {
  vec3 screenCol = texture2D(screenTexture, vUv).rgb;

  // Transmission-only base: retain screen color brightness
  float transmission = glassOpacity;
  vec3 base = screenCol * transmission;

  // 2) chromatic aberration
  vec3 chroma = applyChromaticAberration(screenTexture, vUv, chromaStrength,
                                         chromaBlack, chromaWhite,
                                         redShift, greenShift, blueShift);
  base = blendScreen(base, chroma);     // or blendAdd/blendMultiply…

  // 3) reflections
  vec3 refl = applyReflection(envMap, vNormal, normalize(cameraPosition - vPosition),
                              reflectionStrength, reflectionClamp, fresnelPower);
  base = blendAdd(base, refl, 1.0);

  // Always write out a fully opaque alpha so we only vary color transmission
